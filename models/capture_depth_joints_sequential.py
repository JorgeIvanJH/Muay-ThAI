...
def open_video_capture(capture_source):
    if isinstance(capture_source, int) and sys.platform.startswith("win"):
        # DirectShow is usually more stable than OpenCV's default MSMF webcam backend.
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run metric MDE and YOLO pose tracking from a single capture stream."
    )
    parser.add_argument(
        "--source",
...
def run_combined_capture(args, output_paths):
    ...
    depth_model = mdeutils.safe_model_load(
        args.depth_model,
        use_half=args.half
    )
    mdeutils.configure_pipeline_for_inference(depth_model, input_size=args.input_size)
    print(f"Depth model loaded on device: {args.depth_device}")

    cap = open_video_capture(capture_source)
    if not cap.isOpened():
    ...
    with (
        output_paths["yolo_predictions"].open("w", encoding="utf-8") as yolo_file,
        output_paths["depth_predictions"].open("w", encoding="utf-8") as depth_file,
    ):
        while cap.isOpened():
            ...
            yolo_started_at = time.time()
            yolo_results = yolo_model(frame, verbose=False, device=args.yolo_device)
            yolo_finished_at = time.time()
            ...
            depth_finished_at = time.time()

            depth = depth_result["predicted_depth"].detach().cpu().numpy()
            ...
            people = (
                yoloutils.keypoints_to_people(yolo_results[0])
                if args.save_frame_joints
                else []
            )
            yolo_record = {
                ...
                "joints": {
                    ...
                    "people": people,
                    "boxes": yoloutils.boxes_to_detections(yolo_results[0]),
                },
                "inference": {
                    "model": str(args.yolo_model),
                    "device": args.yolo_device,
                    "started_at": yolo_started_at,
                    "finished_at": yolo_finished_at,
                },
            }
            yolo_file.write(json.dumps(yolo_record) + "\n")
            ...
            depth_record = {
                ...
                "inference": {
                    ...
                    "device": args.depth_device,
                    "started_at": depth_started_at,
                    "finished_at": depth_finished_at,
                },
            }
            depth_file.write(json.dumps(depth_record) + "\n")
...
def main():
    args = parse_args()
    validate_args(args)
    _, source_label = modelutils.parse_source(args.source)
    ...
    run_combined_capture(args, output_paths)
