...
DEFAULT_WEIGHTS = (
    ...
    / "weights"
    / "lightgbm_action.joblib"
)


def load_runtime(weights: Path) -> ActionModelRuntime:
...
def main() -> None:
    parser = build_inference_parser(
        description="Run LightGBM action inference at 30 FPS CFR.",
        default_weights=DEFAULT_WEIGHTS,
    )
    args = parser.parse_args()
    runtime = load_runtime(args.weights)
